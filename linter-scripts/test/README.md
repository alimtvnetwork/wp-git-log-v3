...
| 16 | [`test-audit-ai-implementability.sh`](./test-audit-ai-implementability.sh) | 153 Task A4 | §27 slot 34 `audit-ai-implementability.py` CLI surface contract (no-network mode only — never invokes the LLM gateway): `--help` advertises five mode flags incl. `--no-network`; `--no-network` exits 0 on full tree; `--module=<slot>` walks a single module bundle; `--json` produces parseable JSON; non-md walker (slot 11 `spec/11-powershell-integration` schemas/templates) returns ≥18 files per AC-34-06. Mechanically locks the deep-walk + non-md tier-1 walker contract (Lessons #11/#16) without LLM gateway dependency. | 6 | ~5 s | [AC-34-01..06](../../spec/27-spec-toolchain/34-audit-ai-implementability.md) |
 ...
 | 19 | [`test-ac80-laravel-wp-endpoint-parity.sh`](./test-ac80-laravel-wp-endpoint-parity.sh) | 153 (LBR-12 closure) | spec/22 AC-80 + AC-81 + AC-82 graduation per Lesson L21 / Phase P47–P49 chain: parses `spec/22-git-logs-v2/04-rest-api-endpoints.md` Endpoint Map + `spec/22-git-logs-v2/40-laravel-endpoint-definition.md` Route file Layout (§2), asserts both sides yield exactly 8 distinct (verb,path) pairs (10 logical endpoints collapse to 8 paths — `?q=…` is a query param, not a route), pairwise parity in both directions, AC-81 `DB::connection(...)->getPdo()` citation present, AC-81 `Model::create()` FORBIDDEN citation present, AC-82 Sanctum citation present, AC-82 Passport-forbidden citation present, both `gl.lane-a` + `gl.lane-b` middleware groups declared. Pure read-only parse (no fixtures touched per Lesson #31). | 10 | ~0.2 s | spec/22 AC-80 + AC-81 + AC-82 |

**Totals:** 19 scripts · 197+ assertions · ~44 s of CI time.
