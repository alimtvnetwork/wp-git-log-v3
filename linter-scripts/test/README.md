...
| 13 | [`test-inline-code-blanking-parity.sh`](./test-inline-code-blanking-parity.sh) | P45 | JS↔Python inline-code blanking parity: `linter-scripts/check-spec-cross-links.py::strip_inline_code()` and `linter-scripts/generate-dashboard-data.cjs::blankInlineCode()` MUST produce byte-identical output for any input line, with same-length space runs (char offsets preserved → line numbers stay accurate). 8-fixture corpus probes the P44 root-cause pattern (`` [`./test-foo.sh`](./test-foo.sh) ``), bare/multi/triple-backtick spans, link-adjacent code, and empty input. Floor: fixture count ≥ 8. Mechanically locks the AC-11-05 dual-implementation parity contract (P44 closure). | 17 | ~1 s | [AC-11-05](../../spec/27-spec-toolchain/11-generate-dashboard-data.md) |
