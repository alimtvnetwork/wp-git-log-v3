...
### AC-80 — Laravel Endpoint Binding Parity (Lesson #36 + Lesson #37 — framework-binding link-don't-restate)  `[high]`

**Given** spec/22's REST contract has historically been bound to one framework only (WordPress, via `04-rest-api-endpoints.md`), and a downstream Laravel consumer requires a second framework binding,

**When** a contributor authors a Laravel binding (or any future per-framework binding — Symfony slot 41, etc.),

**Then** the binding file MUST live at a new top-level slot in spec/22 (Laravel = slot 40, file `40-laravel-endpoint-definition.md`; file-slot immutability per Core memory rule applies — no reuse of vacant slots 09–13), AND it MUST be a **binding-only** document: it MUST NOT define new endpoints, new error codes, new validation rules, new DDL, or new auth semantics; every such element is owned upstream and MUST be cited via a cross-module anchor table mirroring AC-79's structure (Lesson #36 link-don't-restate);

**AND** the binding file MUST anchor (at minimum) the following 8 cross-module / intra-module contracts via explicit table rows with "Restate-here forbidden? = YES" flags: (1) endpoint inventory + verb/path/shape → `04-rest-api-endpoints.md`; (2) error envelope → `spec/03-error-manage/00-overview.md`; (3) `GL-*` codes → `15-error-codes.md`; (4) auth lanes + validation order → `05-auth-and-validation.md` + `31-ssh-key-auth.md`; (5) DDL + boolean storage convention → `spec/04-database-conventions/02-schema-design.md` §2.1; (6) SQLite locking → spec/13 §97 AC-22 (via AC-78 inline citation); (7) split-DB log storage → `39-split-db-log-storage.md` + `spec/05-split-db-architecture/00-overview.md`; (8) language coding guidelines → `spec/02-coding-guidelines/03-{php|ts|go|rust|cs}/` (the appropriate sub-folder for the binding's host language);

**AND** the binding file MUST include a self-audit "Conformance Audit Summary" section (mirror of `40-laravel-endpoint-definition.md` §7) asserting (at minimum): every `GL-*` code referenced exists in §15 (no new codes), every endpoint path matches §04 verbatim modulo framework prefix, error envelope shape matches spec/03, boolean storage matches spec/04 §2.1, SQLite locking matches spec/13 §97 AC-22, no cross-module restatement (Lesson #36), language coding guidelines satisfied, slot was not previously used (Core memory file-slot immutability);
...
### AC-83 — Laravel binding implementer-ergonomics bundle (closes LBR-04/05/07/09/10)  `[high]`

**Given** the Laravel binding (`40-laravel-endpoint-definition.md`) has structural parity with §04 (AC-80) and locked persistence + auth-driver postures (AC-81 + AC-82), but the Phase 153 confidence-analysis memo (`.lovable/memory/audit/v2-deterministic/phase-153-laravel-binding-confidence-analysis.md`) enumerates five MEDIUM-severity implementer-ergonomics gaps (LBR-04 single-sentence `?q=` resolver, LBR-05 internal `GlValidationException` ⇄ §15 mapping, LBR-07 unsigned domain-service interfaces, LBR-09 single-sentence migration posture, LBR-10 absent test-harness story) — each of which admits multiple conformant implementations, producing silent design-drift across downstream Laravel packages,

**When** a contributor authors or amends `40-laravel-endpoint-definition.md` (or any future per-framework binding sibling — Symfony slot 41, etc.),

**Then** the binding file MUST contain ALL FIVE of the following pinned conventions at the section anchors below (each anchor is normative — moving the content to a different slot is permitted only via a §99 audit-row-bearing edit):

1. **`?q=` shorthand resolver pinned (LBR-04 closure):** a `BaseLaneAFormRequest::prepareForValidation()` method (or per-binding equivalent in non-PHP frameworks) that expands the §04 row #6/#8 URL-style shorthand `q=<host>/<org>/<repo>[@<branch>]` into explicit `RepoUrl` + `Branch` BEFORE `rules()` validation. The shorthand format is normative: URL-style ONLY (NOT base64-JSON, NOT JSON-in-querystring, NOT comma-separated — these alternative encodings were enumerated and rejected per LBR-04's "four-design ambiguity"). Duplicating the resolver across multiple FormRequests, placing it in a global middleware, or inlining it per controller are all FORBIDDEN.

2. **`$errorCodes` mapping convention pinned (LBR-05 closure):** every Lane B FormRequest MUST declare a `protected array $errorCodes` mapping each `rules()` key (or `key.rule_name` for rule-specific overrides) to its canonical `GL-VALIDATION-*` code from §15. `failedValidation()` MUST consume this map, with resolution order (a) `"{key}.{ruleName}"` → (b) `"{key}"` → (c) hard-fail with `GL-INTERNAL-MAPPING-MISSING`. Throwing Laravel's default `ValidationException` is FORBIDDEN. The mapping table is per-FormRequest (not a global config file) — co-location with the `rules()` it documents is normative.

3. **Domain-service interfaces signed (LBR-07 closure):** `LogIngestService`, `ShaRegistryRepository`, `SplitDbWriter` MUST each be declared as a `Gl\Domain\Contracts\` interface with normative method signatures (no concrete bodies). Interfaces MUST be framework-agnostic (no `Illuminate\…` / `Symfony\…` imports, no facade refs, no framework `Request` types) so future bindings can re-use them verbatim. Concrete bindings (`Sqlite\PdoLogIngestService`, etc.) live downstream.

4. **Migration worked example shipped (LBR-09 closure):** the binding MUST include a worked SQL-file migration example (file naming convention `NNN-{slug}.sql`, body convention `BEGIN IMMEDIATE; …; INSERT INTO MigrationMarker …; COMMIT;`), the `gl-migrator` CLI surface (`migrate` / `status` / `verify-hashes`), and an explicit rollback contract (no automatic down-migration — rollback = author a new forward migration). Laravel's built-in `artisan migrate` MUST be explicitly disabled; using `RefreshDatabase` trait or any Laravel-migrator-coupled fixture is FORBIDDEN.

5. **Testing harness story pinned (LBR-10 closure):** the binding MUST declare (a) framework preference (Pest preferred for new files; PHPUnit supported; Dusk forbidden absent SPA surface); (b) fixture-DB split (`:memory:` for unit, file-DB under `storage/testing/` for integration, file-DB + 2-process fork for concurrency); (c) a reference test pattern for the AC-22 retry-with-jitter contract using a 2-process lock-contention fixture; (d) a coverage floor (every Lane B writer endpoint: 1 happy-path + 1 per-`rules()`-key validation-error test + 1 shared AC-22 retry test; every Lane A read endpoint: 1 happy-path + ≥1 `GL-AUTH-*` failure per `gl.lane-a` rejection branch); (e) at minimum the four forbidden test patterns enumerated in slot 40 §10 (mocking `LogIngestService` in integration tests; Eloquent factories for `Log_*`/`AuditTrail`/`AppLink`; `DB::transaction(…)` in test setup; `RefreshDatabase` trait).

**AND** auditor findings against slot 40 citing "`?q=` resolver undefined" / "no `$errorCodes` mapping" / "unsigned domain interfaces" / "no migration example" / "no test harness" MUST be classified as stale-cache artifacts if the cache predates 2026-06-28 (slot 40 v1.2.0 ships date — Lesson #34 cache-vs-source-of-truth) — the five sections are present per §40 §3 / §3.1 / §4.1 / §6.1 / §10.

**AND** any future per-framework binding (Symfony slot 41, etc.) MUST satisfy this AC's five conventions tailored to its host language (e.g. Symfony: `BaseLaneARequest` extending `Symfony\Component\HttpKernel\Event\ControllerArgumentsEvent` for #1; Symfony Validator violation-to-code mapper for #2; same `Gl\Domain\Contracts\` interfaces re-used verbatim for #3 — interfaces are explicitly framework-agnostic; same `gl-migrator` for #4; PHPUnit/Pest for #5).

- **Verifies:** §40 §3 (`$errorCodes` mapping); §40 §3.1 (`?q=` URL-style shorthand resolver); §40 §4.1 (Domain Service Contracts); §40 §6.1 (Migration worked example + `gl-migrator` CLI); §40 §10 (Testing Harness); `.lovable/memory/audit/v2-deterministic/phase-153-laravel-binding-confidence-analysis.md` LBR-04 + LBR-05 + LBR-07 + LBR-09 + LBR-10 (all five MEDIUM rows closed in a single bundle per the memo §4 deferral note "defer to a 'polish slot 40' phase" — this AC is that polish phase). Cross-cutting parity AC-80 + AC-81 + AC-82 mechanically locked by [`linter-scripts/test/test-ac80-laravel-wp-endpoint-parity.sh`](../../linter-scripts/test/test-ac80-laravel-wp-endpoint-parity.sh) per Lesson L21 / Phase P47–P49 (LBR-12 closure). Codifies **Lesson #16** (lift implementer-ergonomics contract surfaces from buried prose into §97-bound tier-1 visibility — same lesson as AC-81/82, applied to ergonomics axis instead of safety axis) + **Lesson #36** (link-don't-restate — every cross-module concern in the five sections links to its authority rather than restating). Until A8 LLM-gateway re-score runs cleanly, this AC declares any "ambiguous/missing implementer convention" finding against the five anchor sections a stale-cache artifact per Lesson #34.
