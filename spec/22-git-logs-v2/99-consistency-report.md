...
**Version:** 3.13.6  
**Updated:** 2026-06-21 (Phase 153 — slot-40 confidence-lift: locked decisions 14 + 15 + AC-81 + AC-82 close LBR-06 + LBR-08)

> **v3.13.6 update (Phase 153 — slot-40 confidence-lift closing LBR-06 + LBR-08):** User request closed: "improve the spec so that the lower confidence gets improved" referencing `.lovable/memory/audit/v2-deterministic/phase-153-laravel-binding-confidence-analysis.md`. Memo enumerated 15 delivery-risk rows for slot 40 (1 HIGH, 7 MEDIUM, 7 LOW). This phase closes the 2 highest-leverage rows in a single bundle per the memo's §4 recommendation. **Resolution**: (a) `07-app-entity.md` v2.2.0 → **v2.3.0** — added Locked decision 14 (Laravel raw-PDO ingest posture; Eloquent + `DB::transaction` + QueryBuilder writes all FORBIDDEN; Lane A reads exempt; rationale cites Eloquent's `BEGIN DEFERRED` vs spec/13 §97 AC-22's `BEGIN IMMEDIATE`; permanent; future framework bindings cannot inherit) and Locked decision 15 (Laravel Lane A auth = Sanctum bearer tokens; Passport + custom brokers + WP-cookie-bridge + JWT libs + `auth:basic` + Breeze/Jetstream session cookies all FORBIDDEN with per-driver rationale; permanent; future framework bindings cannot inherit). (b) §97 v3.12.0 → **v3.13.0** — added **AC-81** `[critical]` (binds locked decision 14; 4 FORBIDDEN pattern classes detectable by phpcs/grep; PERMITTED Lane A read exceptions; canonical `DB::connection('gl_root')->getPdo()` handle accessor + `$pdo->exec('BEGIN IMMEDIATE')` transaction opener) and **AC-82** `[critical]` (binds locked decision 15; Sanctum `PersonalAccessToken` 1:1 mapping to `Profile`; `gl.lane-a` + `gl.permission:{Perm}` middleware contract with §15 rejection codes; 6 FORBIDDEN auth driver classes). (c) `40-laravel-endpoint-definition.md` v1.0.0 → **v1.1.0** — §5 + §6 prose updated to cite locked decisions 14 + 15 explicitly (closes the LBR-06/LBR-08 root cause: posture was asserted in prose without a locked-decision anchor). AC count 75 → **77**. **Conformance re-audit (performed at authoring + verified here)**: (1) AC-81 + AC-82 cite spec/13 §97 AC-22 + §05 Lane A revoke parity rather than restating → ✅ Lesson #36 honored; (2) both ACs lift contract surfaces out of buried §40 prose into §97-bound tier-1 visibility → ✅ Lesson #16 honored; (3) no new endpoint paths / `GL-*` codes / DDL invented → ✅ verified; (4) §07 locked-decision-14 expanded the forbidden list with concrete Laravel idioms (`DB::transaction`, QueryBuilder writes, model events) — necessary for code-review enforceability, not a contract restatement → ✅; (5) §07 locked-decision-15 enumerated 6 forbidden driver classes with per-driver rationale → ✅ no Sanctum behaviors restated, only the negative space pinned. **Confidence impact** (per memo §4 projection): slot-40 delivery confidence ~70 % → **~85 %** (single HIGH closed + highest-confidence MEDIUM closed); structural confidence ~88 % unchanged. **Remaining LBR rows** (per memo §3 categorisation): 0 HIGH (LBR-06 closed), 6 MEDIUM (LBR-04 `?q=` resolver, LBR-05 `GlValidationException` mapping, LBR-07 `LogIngestService` signatures, LBR-09 migration worked example, LBR-10 test harness, LBR-12 conformance test script), 7 LOW (LBR-01/02/03/11/13/14/15) — all classified "defer to first downstream package author" or "cosmetic". Banners: §97 v3.12.0 → **v3.13.0** (minor — 2 new ACs); §07 v2.2.0 → **v2.3.0** (minor — 2 new locked decisions); §40 v1.0.0 → **v1.1.0** (minor — body cites new locked decisions); §00 v3.13.5 → **v3.13.6** (banner only — patch); §98 v3.13.5 → **v3.13.6** (banner + row — patch); this file v3.13.5 → **v3.13.6** (banner + this audit row — patch). **No CI workflow change · no RUBRIC bump · no AC-31-31 cascade · no gate-count change · no DDL change · no schema bump · no §15 code added · no §05 step added.** **Lesson #36 fourth-instance reinforcement** (AC-81 links to spec/13 AC-22; AC-82 links to §05 Lane A revoke parity). **Lesson #16 second-instance reinforcement** (both ACs lift contract surfaces from buried prose to §97 tier-1 visibility).
