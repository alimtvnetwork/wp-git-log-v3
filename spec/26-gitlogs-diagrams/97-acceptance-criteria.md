...
| AC-D-02 | `02-domain-design.mmd` shows top-down hierarchy GitProfile → Repo → RepoVersion → History/PipelineAction with App linkable to either Repo or GitProfile via AppLink. v3.8.0: also shows the split-DB boundary (root DB subgraph vs per-SHA SQLite subgraph) and `SystemEvent` as Profile-attributed sibling. | brief §Domain.1 + v3.8.0 |
