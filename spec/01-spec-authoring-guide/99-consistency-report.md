...
> **v4.5.0 update (Phase 97 — mermaid syntax gate):** New CI gate `linter-scripts/check-mermaid-syntax.mjs` parses every `spec/**/*.mmd` file (106 at ship time) with the official `mermaid` library under a `jsdom` + `DOMPurify` shim. Found and fixed two latent syntax errors pre-existing in the tree: (1) `lifecycle-spec-authoring.mmd` had a bare `%%` line in its header that mermaid mis-tokenised as a `%%{init}%%` directive (changed to `%% --`), and (2) `spec/12-cicd-pipeline-workflows/images/ci-pipeline-flow.mmd` had unquoted `@` chars in node labels (`B[actions/checkout@v6]` → `B["actions/checkout@v6"]`). Wired into the workflow as a new step after the Phase 95 determinism test. Added a `bun install --frozen-lockfile` step earlier in the workflow (mermaid + jsdom were already devDependencies in `package.json`). Added **AC-SAG-24** in §97 documenting the contract + three common author mistakes + the determinism guarantee. Lockstep: §97 v4.2.0 → v4.3.0; §98 v4.7.0 → v4.8.0. Final result: 106/106 `.mmd` files parse cleanly. No score regression — §01 already had `has_mermaid` bonus from the existing diagram; final score holds at 97/100 A+ with impl=100.
