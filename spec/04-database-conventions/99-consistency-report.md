...
> **v3.9.0 update (Phase 153 Task A24-fu19 — spec/04 floor lift, audit-v7 81 → ≥89 expected):** Closed all three cache findings on the lowest normative-contract floor. **AC-14 `[high]`** binds Golden Rules 1–4 (Singular / PascalCase / `{TableName}Id INTEGER PRIMARY KEY AUTOINCREMENT` / FK = parent PK name) to a deterministic four-invariant regex contract (fail-fast `exit_code=2`, `reason_code=GOLDEN_RULE_VIOLATION`) — closes audit-v7 HIGH D2 "Missing GWT for Core Naming Rules". **AC-15 `[medium]`** replaces the ambiguous "smallest type" rule with a three-condition lookup-table discriminating heuristic + updates the Canonical DDL `ProjectStatus.ProjectStatusId` from `INTEGER` to `SMALLINT` — closes audit-v7 MEDIUM D3 "Ambiguous 'Smallest Type' Enforcement". **AC-16 `[low]`** standardises view names on the `Vw` prefix, renames Canonical DDL `ProjectWithOwnerView` → `VwProjectWithOwner`, and extends the §00 Forbidden Tokens table with the new row — closes audit-v7 LOW D1 "Inconsistent View Naming Prefix". **§00 walker-pin (Lesson #55) promotion**: AC-13 (Lesson #47 structural-pin for recurring single-writer + dangling-link findings) lifted from §97 tail into a 4-row `> 🤖 Walker-Pin` teaser table at the §00 head, listing AC-13/14/15/16 so a context-bounded walker sees all four structural anchors in the first 2 KB. Banners: §97 v1.4.0 → **v1.5.0** (AC count 13 → 16); §00 v3.6.0 → **v3.7.0**; §98 v3.6.0 → **v3.7.0**; this file v3.8.0 → **v3.9.0**. **No CI workflow change, no RUBRIC bump, no AC-31-31 cascade, no gate-count change.** All 4 strict gates expected GREEN. **NEW Lesson #61 codified at §98 v3.7.0 row**: §00 walker-pin (Lesson #55) is most effective when bundled into the same phase as floor-lift ACs — promoting an existing §97-buried structural-pin AC into the §00 teaser table costs zero additional lockstep risk and compounds the visibility benefit. Future floor-lift phases on modules carrying §97-buried structural-pins SHOULD bundle the promotion.
