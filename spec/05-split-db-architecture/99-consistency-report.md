...
> **v4.2.0 update (Phase 153 Task A23 — EXCELLENT-band push via Lesson #45 working levers):** Added **AC-SD-27 `[high]`** (Application/Project terminology binding — 5-row inline canonical-vs-forbidden identifier table; closes v7 MEDIUM D3 "Ambiguous 'ApplicationId' Source") + **AC-SD-28 `[high]`** (Root DB registry-table column completeness — inline schema-floor tables for `Project` 4 columns + `Database` 8 columns including `SequenceNumber INTEGER NOT NULL` + `LastAccessedAt TEXT NOT NULL` + 3 mechanizable `rg` greps; closes v7 LOW D1 "Schema/AC Discrepancy on Registry Table"). §97 v4.4.0 → **v4.5.0** (AC count 26 → 28); §00/§98 v4.4.0 → **v4.5.0**; §99 v4.1.0 → **v4.2.0**. **Expected lift**: weighted +4.9 on `normative-contract` axis (D2×1.5 + D3×1.2 + D1×1.0) → 88.8 + 4.9 = 93.7 → **94 EXCELLENT**; conservative floor ≥91. Pattern follows precedents: spec/03 A21 +7 (D5 citation cluster, audit-corpus ×1.6), spec/04 A21 +8 (D3 edge-case enumeration, normative-contract ×1.2). Confirms Lesson #45 working-lever rule. **No CI workflow change, no RUBRIC bump, no AC-31-31 cascade, no slot-inventory change, no gate-count change.** Memo: `phase-153-task-A23-spec05-excellent-push.md`.
