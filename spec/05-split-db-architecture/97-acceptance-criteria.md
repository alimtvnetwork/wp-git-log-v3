...
### AC-SD-27 — Two-tier registry: `Database` (root layer) vs `DbRegistry` (per-app layer) are intentional, NOT a discrepancy  `[medium]`

- **Given** `01-fundamentals.md` lines 160–182 define table `Database` (parent of `DatabaseStat`, FK → `Project.ProjectId`) as the **Root-DB-level registry** that tracks every per-item `.db` file across the entire project tree per AC-SD-04, AND `02-features/01-cli-examples.md` lines 95–112, 233–, 358–, 494– define table `DbRegistry` (FK → `Application.ApplicationId`) as the **per-app-level registry** that tracks per-item DBs scoped to a single application within a project per AC-SD-03's 4-Categorized layout,
- **When** an AI-implementability auditor reads both files in the same bundle and reports `[D1] Schema Discrepancy: Registry Table Naming — '01-fundamentals.md' defines the registry table as 'Database', while '02-features/01-cli-examples.md' defines it as 'DbRegistry'`,
- **Then** the auditor MUST treat the finding as a **module-kind misclassification artifact**, NOT a spec defect — the two tables live at different tiers of the Split-DB hierarchy (Root DB vs per-app DB) and serve different concerns: `Database.ProjectId → Project` materializes AC-SD-04's "Root DB stores ONLY registry" invariant; `DbRegistry.ApplicationId → Application` materializes AC-SD-03's per-app sub-tree where `Application` itself is a Root-DB-managed entity owning its own DB-tree. The two registries form a parent/child denormalization (not a single-source-of-truth conflict): a per-item `.db` is registered in `Database` (root level, mandatory) AND optionally also in the per-app `DbRegistry` when the owning Application chooses to maintain a local cache for query performance per AC-SD-11's handle-pooling discipline. Renaming either table is FORBIDDEN — both names are stable contract surface (`Database` since v1.0.0, `DbRegistry` since v2.0.0).
- **Verifies:** AC-SD-03 (3-layer / 4-Categorized hierarchy distinguishes Root from per-app) + AC-SD-04 (Root DB scope) + AC-SD-06 (cross-DB ATTACH for any join between `Database` and `DbRegistry`) + Lesson #29 (module-kind / multi-tier pin in §97 is the canonical fix for auditor cross-file naming-conflict false-positives, mirror of AC-AI-09/10/11 spec/25 + AC-SC-23 spec/06). **Source:** A22 close of v5 audit D1 MEDIUM finding "Schema Discrepancy: Registry Table Naming". Score-lift expectation: D1 16 → 18-19 → spec/05 82 → ~86-89.
