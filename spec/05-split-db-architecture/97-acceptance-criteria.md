...
### AC-SD-27 — Polyglot connection-discipline cross-reference (PRAGMA + driver examples live in spec/13)  `[medium]`

- **Given** AC-SD-22 mandates `PRAGMA busy_timeout=5000` + retry-loop on `SQLITE_BUSY`/`SQLITE_LOCKED` for any process opening a Split-DB connection (Go, Python, TypeScript, Rust, PHP, C# — language-agnostic), AND `01-fundamentals.md` § "SQLite WAL Mode" exhibits ONLY a Go fragment (`db.Exec("PRAGMA journal_mode=WAL")` + `db.Exec("PRAGMA busy_timeout=5000")`),
- **When** an AI implementer or non-Go reader opens this module looking for the canonical multi-driver connection-string + PRAGMA discipline (Python `sqlite3` / TypeScript `better-sqlite3` / Rust `rusqlite` / PHP `PDO_SQLITE` / C# `Microsoft.Data.Sqlite`),
- **Then** the implementer MUST resolve the polyglot driver-specific recipe via the canonical owner `spec/13-generic-cli/10-database.md` § "Concurrency & Locking (Normative)" + § "PRAGMA Discipline" — that file is the single source of truth for the per-driver DSN form, the `BEGIN IMMEDIATE` discipline, the retry-loop with jitter (3×100ms ±25%), and the `~/.local/state/<binary-name>/update.lock` file-lock posture. This module (spec/05) deliberately does NOT inline polyglot driver examples: doing so would create exactly the dual-source drift that Lesson #36 forbids — `spec/13/10-database.md` evolves under spec/13 §98 and any copy here would diverge silently. The Go fragment in `01-fundamentals.md` is RETAINED as one illustrative example only; the contract is canonical at spec/13.

- **Forbidden patterns:** inlining a Python/TS/Rust/PHP/C# connection-string recipe into spec/05 prose (creates dual-source drift under Lesson #36); citing the Go fragment as the normative form for non-Go languages (it is one example among many — the contract is the PRAGMA + retry-loop semantics, not the Go-specific `db.Exec` syntax); altering AC-SD-22's wording to claim driver-specific completeness (it is deliberately language-agnostic).
- **Verifies:** AC-SD-22 cross-process concurrency (the language-agnostic contract surface in this module) + spec/13 AC-22 (the canonical concurrency owner per Phase 153 P3 prose-mirror) + spec/13 § "Concurrency & Locking (Normative)" implementer prose (the polyglot driver recipe). **Source:** A05-fu close of v7 audit MEDIUM/D3 finding "Missing Concurrency Handling for Non-Go Languages" via Lesson #36 link-don't-restate cross-module pin (mirror of AC-SD-24 for the inheritance axis applied here to the polyglot-driver axis). Mirror of spec/04 §4.3 Concurrency Posture (Normative cross-reference) — same pattern, same canonical owner, different consumer module.
