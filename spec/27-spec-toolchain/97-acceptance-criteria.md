...
### AC-T-35 — Trace-map auto-proposer (slot 14 `generate-trace-map.py`) [medium]
- **Given** the bijection rule (AC-T-01) requires every executable artifact under `linter-scripts/` to appear in exactly one `[[trace]]` entry of `linter-scripts/trace-map.toml`, and the current authoring flow is hand-edited (cited in G-series memos as "mechanical 1:1 work" that produces ~25 long-tail orphans),
- **When** slot 14 is invoked with the proposed `--propose-missing` flag (spec-only — script work deferred per spec-only-mode preference),
- **Then** the script MUST: (a) enumerate the orphan set `O = code(linter-scripts/) − files(trace-map.toml)`; (b) for every `o ∈ O`, emit a draft `[[trace]]` block to STDOUT (NEVER to disk — auto-write is FORBIDDEN, human review is mandatory) with `ac = "TODO/<best-guess-spec-path>#TODO-AC-ID"`, `files = ["<orphan-path>"]`, `symbols = []` (filled by AC-T-37 extractor when available), `note = "auto-proposed by --propose-missing on <UTC-timestamp> — replace TODOs before committing"`; (c) exit `0` (proposals are advisory, not errors — orphan errors stay under AC-14-03); (d) best-guess AC mapping uses substring match of orphan basename against existing `spec/27-spec-toolchain/NN-*.md` slot numbering (e.g. `check-foo.py` → `spec/27-spec-toolchain/0X-foo.md`) and emits `# UNRESOLVED` comment when no candidate exists.
- **AND** the proposer is **idempotent and read-only** vs the TOML file (writes only to STDOUT); re-runs on a satisfied tree print `# trace-map.toml: 0 orphans, nothing to propose` and exit `0`.
- **Detection (spec-only-mode operational substitute per Lesson #93):** auditors verifying AC-T-35 compliance MUST run `comm -23 <(find linter-scripts -maxdepth 1 -type f \( -name '*.py' -o -name '*.cjs' -o -name '*.sh' -o -name '*.ps1' -o -name '*.go' \) | sort) <(grep -hE '^\s*files\s*=' linter-scripts/trace-map.toml | grep -oE '"linter-scripts/[^"]+"' | tr -d '"' | sort -u)` and confirm the resulting orphan list matches the trace-map regression baseline (`code_orphan: 25` per Phase H1 rebaseline in `mem://specs/full-tree-audit-v4`).
- **Verifies:** `linter-scripts/trace-map.toml` (target write surface — proposer reads only); `linter-scripts/generate-trace-map.py` (host script — `--propose-missing` flag to be added); `spec/27-spec-toolchain/14-generate-trace-map.md` §3 "CLI flags" (flag-surface contract); AC-T-01 (bijection rule this proposer accelerates); codifies the G-series lesson "AC binding is mechanical 1:1 work" (mem://index.md G-series CLOSED block) by lifting the mechanical work into tooling.

### AC-T-36 — OpenAPI/Swagger trace metadata export (slot 14 `generate-trace-map.py`) [medium]
- **Given** modules with HTTP-endpoint contracts (spec/22 §04 WordPress REST, spec/22 §40 Laravel routes, future Symfony/Slim bindings under the `framework-binding` axis introduced in slot 34 v2.92.0) need machine-discoverable cross-references between an OpenAPI operation (`<METHOD> <PATH> operationId=foo`) and its governing AC,
- **When** slot 14 is invoked with the proposed `--openapi-out=<path>` flag (spec-only — script work deferred),
- **Then** the script MUST: (a) accept optional `endpoint = "<METHOD> <PATH>"` and `operation_id = "<id>"` keys on `[[trace]]` entries (TOML schema extension to slot 14 §6); (b) when `--openapi-out` is set, emit OpenAPI 3.1 fragments `{"paths": {"<PATH>": {"<method>": {"x-spec-ac": "<spec-relpath>#<ac-id>", "x-trace-source": "linter-scripts/trace-map.toml", "operationId": "<id>"}}}}` into the target file as a single JSON document with sorted keys (determinism per AC-14-02); (c) skip entries missing both `endpoint` and `operation_id` (silent — these are non-HTTP traces, NOT errors); (d) FORBID writing `info` / `servers` / `components` sections — this script is a **metadata exporter only**, NOT a full OpenAPI generator (full schema lives downstream in the implementation repo, consumed by `php artisan route:list --json` or `wp rest list` and merged).
- **Detection (spec-only-mode operational substitute per Lesson #93):** reviewers MUST verify, by reading `spec/22-git-logs-v2/04-wp-endpoint-definition.md` §3 + `spec/22-git-logs-v2/40-laravel-endpoint-definition.md` §3, that the 8 distinct endpoint paths enumerated by `linter-scripts/test/test-ac80-laravel-wp-endpoint-parity.sh` (LBR-12 closure) each appear as a `[[trace]]` entry candidate with the future `endpoint` key set to the verb/path tuple — the AC-80 parity test is the canonical input set this exporter consumes.
- **AND** the exporter MUST NOT couple to any framework runtime — it reads ONLY `linter-scripts/trace-map.toml` + the spec tree, never executes PHP / Composer / `artisan` / `wp` CLI (decoupling preserves Lesson #36 "cross-module cross-references MUST link, never restate" — the spec is the single source).
- **Verifies:** `linter-scripts/trace-map.toml` (TOML schema extension — `endpoint` + `operation_id` keys); slot 14 §6 "TOML schema" (contract for the new keys); slot 14 §4 "CLI flags" (new `--openapi-out` flag); `spec/22-git-logs-v2/40-laravel-endpoint-definition.md` AC-83 (sibling-of WP §04 — the 8-endpoint set this export covers); slot 34 `framework-binding` axis (Phase 153 hygiene-round addition — establishes future Symfony/Slim consumers).

### AC-T-37 — Endpoint/function-name extraction for sub-file trace targets (slot 14 `generate-trace-map.py`) [medium]
- **Given** code targets larger than ~500 lines (e.g. `linter-scripts/audit-ai-implementability.py` ~900 lines / `linter-scripts/check-tree-health.cjs` ~600 lines) carry multiple distinct invariants and the current `[[trace]]` schema points only at the **file** (`files = [...]`) without sub-file precision — making it impossible to trace `AC-34-14 (walker cap)` to a specific function vs `AC-34-17 (chunked re-scoring)` in the same file,
- **When** slot 14 is invoked with the proposed `--extract-symbols` flag (spec-only — script work deferred),
- **Then** the script MUST: (a) treat the existing `symbols = ["funcName", ...]` array on `[[trace]]` entries (slot 14 §6 TOML schema, currently optional) as **first-class trace targets** — each symbol resolves to a sub-file location (file + line-range); (b) validate that every declared `symbols` entry exists in the target file by extracting symbol definitions per language using AST-free regex: Python `^def <name>\(|^class <name>[ (:]` ; JavaScript/CJS `^(function|async function|const|class) <name>\b` ; Shell `^<name>\(\)` ; PowerShell `^function <name>\b` ; Go `^func (\([^)]+\) )?<name>\(` — emit `## ❌ Errors → Missing symbols` table entry and exit `1` per AC-14-04 pattern when a symbol cannot be located; (c) emit symbol+line-range pairs into `.lovable/memory/audit/trace-map.json` under a new `symbols_resolved` array per `[[trace]]` entry (`{"file": "...", "symbol": "...", "line_start": N, "line_end": M}`); (d) FORBID multi-symbol matching ambiguity — if a symbol name appears twice in the same file (overloads, nested functions), the proposer MUST require disambiguation via `symbols = ["funcName@<line-hint>"]` or fail with `## ❌ Errors → Ambiguous symbols`.
- **Detection (spec-only-mode operational substitute per Lesson #93):** reviewers MUST verify, by spot-checking 3 `[[trace]]` entries with `symbols` populated (per G-series memo any of the 14 Phase-117-absorbed scripts), that running `grep -nE '^(def |^class |^function |^func )<symbol-name>' <file>` returns exactly one match per declared symbol — the regex set above MUST round-trip with grep output on the live tree.
- **AND** the extractor is **read-only against source** — it MUST NOT rewrite `[[trace]]` entries; it MAY emit a `# suggested symbols:` STDOUT comment block (parallel to AC-T-35's STDOUT-only contract) when invoked with `--propose-missing --extract-symbols` together (the two flags COMPOSE: proposer drafts entry, extractor drafts symbol list).
- **AND** sub-file precision is **opt-in**: traces without `symbols` continue to work under file-level granularity (Lesson #18 honest-baseline — do NOT silently widen the contract for existing entries).
- **Verifies:** `linter-scripts/trace-map.toml` `symbols` key (slot 14 §6 TOML schema — currently optional, this AC promotes it to validated when present); slot 14 §4 "CLI flags" (`--extract-symbols` flag); slot 14 §5 "Outputs" (new `symbols_resolved` JSON sub-array contract); `linter-scripts/audit-ai-implementability.py` + `linter-scripts/check-tree-health.cjs` (canonical multi-AC-per-file targets demonstrating the need); AC-T-01 (bijection rule — this AC tightens "code artifact" granularity from file to file+symbol); AC-T-35 (proposer composition partner); codifies the G-series open observation "remaining ~25 orphans are run.sh/deprecated-v1/helpers/audit-internals — R1 is the proper deeper-binding tool" (mem://index.md) by giving R1 a deterministic symbol-resolution sub-contract that survives without Lovable Cloud (gateway-blocked).
