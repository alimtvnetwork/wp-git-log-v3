...
### AC-34-19 — Cache-staleness banner: warn when `bundle_sha` predates the module's §98 latest row `[high]`
- **Given** the audit-cache schema persists one JSON file per module under `.lovable/cache/audit-ai/<module>.json` carrying a `bundle_sha` (16-hex content hash) and a `scored_at` (ISO-8601 UTC timestamp), AND each module's `98-changelog.md` carries a `**Updated:** YYYY-MM-DD` banner at line 4 plus a topmost dated row `### X.Y.Z — YYYY-MM-DD — <summary>`, AND Lesson #34 codified that audit caches "go stale on every spec patch; until gateway unblocks, the cache MUST NOT be the authoritative source of CRITICAL counts" — yet today nothing in the toolchain emits a visible signal when a CRITICAL finding is read from a cache file scored *before* a §97/§98 edit landed (silent stale-CRITICAL re-surfacing is the failure mode this AC closes),
- **When** any consumer of `.lovable/cache/audit-ai/<module>.json` renders the cached findings to a human reviewer — including (a) `audit-ai-implementability.py --report-only`, (b) the scorecard table rendered at the top of every spec-improvement response per the Core scorecard-display rule, (c) any future dashboard generator (slot 11 `generate-dashboard-data.cjs` extension) that surfaces audit-ai findings, and (d) any human inspecting the JSON file directly via `cat` / `jq`,
- **Then** (a) the cache JSON MUST carry a top-level `stale_vs_changelog: <bool>` field set to `true` when `cache.scored_at < module.98_changelog.latest_row.date` (date-level comparison; the §98 row date is authoritative — banner `**Updated:**` is a mirror per Lesson #25 and MUST NOT be the comparator's primary input); (b) when `stale_vs_changelog == true`, the cache JSON MUST additionally carry `stale_reason: "scored {scored_at} · §98 latest row {latest_98_row_version} dated {latest_98_row_date} ({days_behind}d behind)"` so reviewers see the gap without re-parsing the changelog; (c) every renderer enumerated in the **When** clause MUST emit a one-line banner immediately above the findings list of the form `⚠️  STALE-CACHE: <module> scored {scored_at}; §98 advanced to {latest_98_row_version} on {latest_98_row_date} ({days_behind}d behind) — discount CRITICAL/HIGH findings until gateway re-score (Lesson #34)` — the banner MUST be visually distinguishable (emoji or `[STALE]` prefix; plain-text `WARNING:` is insufficient because it collides with LLM-finding bodies that contain the word "warning"); (d) **Detection** (for human auditors operating in spec-only mode pending script implementation): grep the cache file with `python3 -c "import json,sys; d=json.load(open(sys.argv[1])); print(d.get('stale_vs_changelog','MISSING'))" .lovable/cache/audit-ai/<module>.json` — any module returning `MISSING` predates this AC and any module returning `True` MUST have its CRITICALs discounted in the current response's scorecard table; (e) cache invalidation thresholds are NORMATIVE: `stale_vs_changelog` flips `true` when `days_behind ≥ 1` (any §98 row dated strictly after `scored_at` triggers the banner — no grace period because §97/§98 lockstep means every changelog row signals a contract-surface change per Lesson #25); (f) the banner MUST NOT be suppressible via a CLI flag (mirror of AC-34-08 advisory-by-default rule: caches are advisory inputs, but their staleness is a normative output); (g) **spec-only-mode interim posture** (2026-06-28 directive): the cache schema extension (`stale_vs_changelog` + `stale_reason` fields) AND the renderer banner emission are both implementation work deferred per backlog item `A8-cache-staleness-banner` — until the script ships, the **Detection** clause is the operational surface (every spec-improvement response's scorecard table MUST manually compute `days_behind` for any module whose cache is read and prepend the `⚠️  STALE-CACHE:` line to flagged rows); this manual-substitute is mandatory, not optional, and is the spec-only-mode equivalent of the mechanical-lock per Lesson #92.
- **Verifies:** §34 cache-trust contract — closes the Lesson #34 "stale CRITICAL re-surfacing" class by making staleness a first-class field in the cache schema + a mandatory rendering surface in every cache consumer. Mirrors AC-34-09's "tier-1 contract files MUST be visible to the auditor" discipline at the meta-axis (the *staleness of the audit itself* MUST be visible to the human reviewer). Pairs with `.lovable/question-and-ambiguity/README.md` "Closure protocol" (Phase 153 hygiene-round-3) — both address Lesson #32's "anchor at source so verification is possible" pattern: §97/§98 dates are the anchors, cache fields are the verification surface. **NEW Lesson #93** (codified inside this AC's clause (g)): in spec-only mode, every script-deferred AC MUST carry an explicit **Detection** clause that prescribes a manual operational substitute (grep recipe, scorecard-row computation, human-eye check) — without the substitute, the AC ships as text-only and the contract gap stays open until the script lands (potentially multiple phases away). Detection clauses are the spec-only-mode counterpart of mechanical-lock self-tests (Lesson #31 / Phase P47–P49 graduation pattern) and graduate to script gates when the spec-only directive lifts.
