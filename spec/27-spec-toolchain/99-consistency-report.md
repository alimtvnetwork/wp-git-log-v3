...
| Date | Version | Action |
|------|---------|--------|
| 2026-05-04 | 2.86.1 | Phase 153 Task A18-impl-2: AC-34-16 codifies intra-T1 splitting (anchor-pair `00+97` always together when T1>MAX_BYTES) + per-chunk SHA cache schema (`chunks: [{tier, bundle_sha_chunk, files, bytes_used}]`). Replaces the pre-impl-2 pathological branch that truncated T1 to first MAX_BYTES bytes (lost 70%+ of contract for spec/27 where T1=455 KB). Composite `bundle_sha` retained for backward parity (Lesson #79 — additive-only field migration). Self-test 14 → 16 assertions. Slot 34 §00 v1.6.0 → v1.7.0; §97 v2.13.0 → v2.14.0; §00/§98 v2.90.0 → v2.90.1; this §99 v2.86.0 → v2.86.1. No CI / RUBRIC / gate-count change. |
