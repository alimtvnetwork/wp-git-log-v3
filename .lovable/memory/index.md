...
Tree health is MEASURED by `node linter-scripts/check-tree-health.cjs --report` — never narrate scores. Current measured baseline: 81/100 (B); CI threshold locked at 80 (v3.7.3). Run `node linter-scripts/generate-spec-index.cjs` after spec changes.
Session-persistence regression CONFIRMED FOURTH TIME (v3.7.2 `check-tree-health.cjs` was deleted between turns within same session) — always re-verify file presence at session start AND between major operations.
...
- [Full-tree audit v4](mem://specs/full-tree-audit-v4) — Pre-triage 45/100 (F). Phase 1 (v3.7.0) renames + archive. Phase 2a (v3.7.3) authored 14 consistency reports → MEASURED 81/100 (B); required files 104/104. Remaining: ~13 missing `97-acceptance-criteria.md` (recommended weight 25%). CI gate threshold 80.
