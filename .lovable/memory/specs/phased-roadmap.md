...
- **Phase 29** — `spec/spec-index.md` regen + advisory-gate root-cause: CLOSED 2026-04-28 (No-Questions Mode 13/40). Per Phase 28 close-out rec (b). Regenerated `spec/spec-index.md` (877→**883 files**, 1042→1048 lines, +6 §27 slots: 18, 19, 25, 26, 27, 28). Flushed 12 stale version entries accumulated since Phase 145+: B1 §07 v2.1→2.2, Phase 147 §97 v3.9.0→3.9.1 + §99 v3.9.5→3.9.6, §27 §00 v1.7.0→**v2.46.3**, §27 §31 v1.22→1.23, §27 §62 v1.0→1.2, §27 §97 v2.0.1→2.2, §27 §98 v2.30→**v2.46.3**, §27 §99 v2.27→**v2.43.3**, root §98 v3.5.0→3.5.1. **Root cause identified**: `.github/workflows/spec-health.yml` line 63 step "Regenerate spec-index.md (drift check)" is **advisory** — runs the regen + `git status --porcelain spec/` but on drift prints `⚠️` warning and `exit 0`. Phases 145, 147, F2/F3, H1, H4–H7, 28 all bumped versions; none ran `bash linter-scripts/run.sh` locally; CI silently rubber-stamped each. Added Phase 29 NOTE comment block to the workflow step explaining the deferred-promotion rationale (no behavior change). Both gates green; QA-baseline-footer 17/17/17 parity preserved (advisory step doesn't count). Memo: `phase-29-spec-index-regen.md`. **Phase 30 QUEUED** = strict-promote + AC-31-31 cascade (RUBRIC v2.26→v2.27, footer 17→18, EXECUTIVE-SUMMARY back-ref, qa-baseline-footer awk, §27 §97 new AC, §27 §98/§99 bumps, Memory Core gate-count update). Following H4→H5 "don't rush AC-31-31" playbook — split artifact regen from rubric cascade. **Lessons**: (1) **advisory CI gates silently rot** — second instance after H1's session-persistence regression class; any gate detecting fixable drift SHOULD `exit 1` unless there's a specific phased-rollout reason it must remain advisory. (2) **Generator artifacts in `run.sh` need CI parity** — `run.sh` has the regen, but contributors don't always run it; CI must regen + diff strictly OR the artifact must not be committed (treat as build output). (3) **AC-31-31 cascade discipline** — treat strict-gate promotion as its own phase to avoid mixing artifact regen with multi-file rubric/footer/contract bumps; H4→H5 split worked; replicate.
